...
WHEEL_FILE:=dist/enterprise_gateway_scheduler_extension-$(VERSION)-py2.py3-none-any.whl
...
clean: ## Make a clean source tree
	...
	-rm -rf build
	-rm -rf *.egg-info
	-find enterprise_gateway_scheduler_extension -name __pycache__ -exec rm -fr {} \;
	-find enterprise_gateway_scheduler_extension -name '*.pyc' -exec rm -fr {} \;
...
.image-docker: etc/docker/* $(WHEEL_FILE)  
	...
	cp dist/enterprise_gateway_scheduler_extension* build/docker
